...
class BaseServer:
    ...
    def parse_cmd_filter(self, data):
        import re
        IGNORE_OUTPUT_COMMAND = [re.compile(r'rm -rf\s?'),
                                 re.compile(r'^ssh\s*'),
                                 re.compile(r'^chpasswd\s*'),
                                 re.compile(r'^passwd\s*')]
        # 输入了回车键, 开始计算输入的内容
        if self._have_enter_char(data):
            self._in_input_state = False
            self._input = self._parse_input()
            # logger.info("##### K8S: before: {}".format(data or ""))
            #if self._input:
            #    logger.info("##### K8S: self._input: '{}'".format(self._input))
            for pattern in IGNORE_OUTPUT_COMMAND:
                if self._input and pattern.match(self._input):
                    logger.info("##### *********** hit!")
                    return data.replace(b'\r', b'\x03\r\n')
            # logger.info("##### K8S: end: {}".format(data or ""))
            return data
    ...
    def send(self, data):
        self.initial_filter()
        data = self.parse_cmd_filter(data)
