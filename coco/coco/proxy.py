...
class ProxyServer:
    ...
    def get_ssh_server_conn(self):
        ...
        data = {"user": self.system_user.name.split("_")[0], "asset": self.asset.ip,
                "system_user": self.system_user.username, "login_type": "ST",
                "is_failed": 0}
        if not chan:

            self.client.send(warning(wr('连接超时，请10秒后重新连接！', before=1, after=0)))
            app_service.reload_secret_key(data)
            server = None
        else:
